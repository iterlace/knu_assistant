...
def change_group(update: Update, ctx: CallbackContext, session: Session, user: User):
    ...
    # TODO: add "end" callback if the user already has a group
    keyboard = build_keyboard_menu(kb_buttons, 4)
    bot.sendMessage(update.effective_user.id, "На якому курсі ти навчаєшся?",
...
def select_course(update: Update, ctx: CallbackContext, session: Session, user: User):
    ...
    bot.sendMessage(update.effective_user.id, "На якому факультеті?",
                    reply_markup=InlineKeyboardMarkup(keyboard))
